...
static void StopSteps(void *context)
{
    ...
    Timer_Periodic_Command(&instance->moveStepTimer, Timer_Periodic_Command_Stop);
    Timer_OneShot_Start(&instance->publishStepsDoneEventTimer);
}
...
static void DoStep(I_StepperMotor_t *_instance, StepDirection_t direction, uint16_t numberOfSteps)
{
    ...
    if(!instance->busy)
    {
        ...
        Timer_OneShot_UpdateExpiryTicks(&instance->stopStepsTimer, numberOfSteps * (uint16_t)MinimumPeriodForStepSubCycleMs);
        Timer_Periodic_Start(&instance->moveStepTimer);
        Timer_OneShot_Start(&instance->stopStepsTimer);
    }
}
...
static void Stop(I_StepperMotor_t *_instance)
{
    ...
    Timer_Periodic_Command(&instance->moveStepTimer, Timer_Periodic_Command_Stop);
    Timer_OneShot_Stop(&instance->stopStepsTimer);
    Timer_OneShot_Start(&instance->publishStepsDoneEventTimer);
}
